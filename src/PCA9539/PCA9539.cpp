...
void PCA9539::allPinMode(uint8_t a_channel, uint8_t b_channel) {
    _configurationRegister_low = a_channel;
    _configurationRegister_high = b_channel;
    I2CSetValue(_address, NXP_CONFIG    , a_channel);
    I2CSetValue(_address, NXP_CONFIG + 1, b_channel);
}
/**
...
void PCA9539::allDigitalWrite(uint8_t a_channel, uint8_t b_channel) {
    _valueRegister_low = a_channel;
    _valueRegister_high = b_channel;
    I2CSetValue(_address, NXP_OUTPUT    , a_channel);
    I2CSetValue(_address, NXP_OUTPUT + 1, b_channel);
}
//
...
uint16_t PCA9539::I2CGetValue(uint8_t address, uint8_t reg) {
    ...
    //
    pcaI2C->beginTransmission(address);          // setup read registers
    pcaI2C->write(reg);
    _error = pcaI2C->endTransmission();
    //
    // ask for 2 bytes to be returned
    //
    if (pcaI2C->requestFrom((int)address, 1) != 1)
    {
        //
...
void PCA9539::I2CSetValue(uint8_t address, uint8_t reg, uint8_t value){
    ...
    //
    pcaI2C->beginTransmission(address);              // setup direction registers
    pcaI2C->write(reg);                              // pointer to configuration register address 0
    pcaI2C->write(value);                            // write config register low byte
    _error = pcaI2C->endTransmission();
    if(_error != 0x00){
        Serial.printf("I2C error : %d\n",_error);
        delete pcaI2C;
        pcaI2C = new TwoWire(0);
        pcaI2C->begin();
    }
}
